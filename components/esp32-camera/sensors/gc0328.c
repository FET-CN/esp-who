...
static int reset(sensor_t *sensor)
{
    ...
    if (ret == 0) {
        ...
        set_reg_bits(sensor->slv_addr, 0xfa, 4, 0x07, 1);  //frequency division for esp32, ensure pclk <= 15MHz
#endif
    }
    return ret;
}
...
static int set_pixformat(sensor_t *sensor, pixformat_t pixformat)
{
    ...
    case PIXFORMAT_RGB565:
        write_reg(sensor->slv_addr, 0xfe, 0x00);
        ret = set_reg_bits(sensor->slv_addr, 0x44, 0, 0xff, 0x26);  //RGB565
        break;

    case PIXFORMAT_YUV422:
        write_reg(sensor->slv_addr, 0xfe, 0x00);
        ret = set_reg_bits(sensor->slv_addr, 0x44, 0, 0xff, 0x22); //yuv422 Y Cb Y Cr
        break;

    case PIXFORMAT_GRAYSCALE:
        write_reg(sensor->slv_addr, 0xfe, 0x00);
        // ret = set_reg_bits(sensor->slv_addr, 0x44, 0, 0xff, 0xb1); //Y only
        // break;
        // write_reg(sensor->slv_addr, 0xfe, 0x00);
        ret = set_reg_bits(sensor->slv_addr, 0x44, 0, 0xff, 0x22); //yuv422 Y Cb Y Cr
        break;
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    while (regs[i][0]) {
        ...
        write_reg(sensor->slv_addr,regs[i][0], regs[i][1]);
//        msleep(1);
        i++;
    }
...
// #elif CONFIG_GC_SENSOR_WINDOWING_MODE
    ...
    // write_reg(sensor->slv_addr, 0x06, col_s / 4);
    // write_reg(sensor->slv_addr, 0x07, row_s / 4);
    // write_reg(sensor->slv_addr, 0x08, (col_s + h) / 4);
    // write_reg(sensor->slv_addr, 0x09, (row_s + w) / 4);
...
static int set_contrast(sensor_t *sensor, int contrast)
{
    if (contrast != 0) {
        write_reg(sensor->slv_addr, 0xfe, 0x00);
        write_reg(sensor->slv_addr, 0xd3, contrast);
    }
    return 0;
}
...
static int set_global_gain(sensor_t *sensor, int gain_level)
{
    if (gain_level != 0) {
        write_reg(sensor->slv_addr, 0xfe, 0x00);
        write_reg(sensor->slv_addr, 0x70, gain_level);
    }
    return 0;
}
...
static int set_colorbar(sensor_t *sensor, int enable)
{
    int ret = 0;
    ret = write_reg(sensor->slv_addr, 0xfe, 0x00);
    ret |= set_reg_bits(sensor->slv_addr, 0x4c, 0, 0x01, enable);
    if (ret == 0) {
...
static int init_status(sensor_t *sensor)
{
    ...
    sensor->status.agc = 0;
    sensor->status.aec = 0;
    sensor->status.hmirror = check_reg_mask(sensor->slv_addr, 0x17, 0x01);
    sensor->status.vflip = check_reg_mask(sensor->slv_addr, 0x17, 0x02);
...
static int set_aec_value(sensor_t *sensor, int val){
    int ret = 0;
    sensor->status.aec_value = MAX(MIN(val,1023),0);
    ret = write_reg(sensor->slv_addr, 0xfe, 0x00);
    ret |= set_reg_bits(sensor->slv_addr, 0x04, 0, 0xff, sensor->status.aec_value&0xFF);
    ret |= set_reg_bits(sensor->slv_addr, 0x03, 0, 0x0f, (sensor->status.aec_value>>8)&0x0F);
    
    if (ret == 0) {
        ESP_LOGD(TAG, "Set aec value to: %d", sensor->status.aec_value);
    }
    return ret;
}

static int set_aec(sensor_t *sensor, int enable){
    int ret = 0;
    sensor->status.aec = enable;
    ret = write_reg(sensor->slv_addr, 0xfe, 0x00);
    ret |= set_reg_bits(sensor->slv_addr, 0x4f, 0, 0x01, enable != 0);
    if (ret == 0) {
...
static int set_whitebal(sensor_t *sensor, int enable){
    ...
    ret = write_reg(sensor->slv_addr, 0xfe, 0x00);
    reg = read_reg(sensor->slv_addr,0x42);
    ret |= set_reg_bits(sensor->slv_addr, 0x42, 0, 0xff, reg&(0xff^(enable?0x0:0x2)));
    if (ret == 0) {
        ESP_LOGD(TAG, "Set awb to: %d", ((read_reg(sensor->slv_addr,0x03)<<8)|read_reg(sensor->slv_addr,0x04))&0xFFF);
    }
    return ret;
}
...
int gc0328_init(sensor_t *sensor)
{
    ...
    sensor->set_exposure_ctrl = set_aec;    //
...
void gc0328_antiflicker(sensor_t* sensor, int mode){
    if(mode==0){    //50hz
		...
		write_reg(sensor->slv_addr,0x06,0x2c);
		write_reg(sensor->slv_addr,0x07,0x00);
		write_reg(sensor->slv_addr,0x08,0xb8);
		write_reg(sensor->slv_addr,0xfe,0x01);
		write_reg(sensor->slv_addr,0x29,0x00);
		write_reg(sensor->slv_addr,0x2a,0x60);
		write_reg(sensor->slv_addr,0x2b,0x02);
		write_reg(sensor->slv_addr,0x2c,0xa0);
		write_reg(sensor->slv_addr,0x2d,0x03);
		write_reg(sensor->slv_addr,0x2e,0x00);
		write_reg(sensor->slv_addr,0x2f,0x03);
		write_reg(sensor->slv_addr,0x30,0xc0);
		write_reg(sensor->slv_addr,0x31,0x05);
		...
		write_reg(sensor->slv_addr,0x07,0x00);
		write_reg(sensor->slv_addr,0x08,0x88);
		write_reg(sensor->slv_addr,0xfe,0x01);
		write_reg(sensor->slv_addr,0x29,0x00);
		write_reg(sensor->slv_addr,0x2a,0x4e);
		write_reg(sensor->slv_addr,0x2b,0x02);
		write_reg(sensor->slv_addr,0x2c,0x70);
		write_reg(sensor->slv_addr,0x2d,0x03);
		write_reg(sensor->slv_addr,0x2e,0x0c);
		write_reg(sensor->slv_addr,0x2f,0x03);
		write_reg(sensor->slv_addr,0x30,0xa8);
		write_reg(sensor->slv_addr,0x31,0x05);
		write_reg(sensor->slv_addr,0x32,0x2e);
		write_reg(sensor->slv_addr,0xfe,0x00);
    }
}